  ...
  return (
    <div style={{ maxWidth: 420, margin: "0 auto", minHeight: "100vh", background: "#080f1a", color: "#fff", paddingBottom: 32 }}>
      ...
      <div style={{ padding: "1.5rem 1.25rem 2rem", position: "relative", overflow: "hidden" }}>
        ...
        <div style={{ fontSize: 13, color: "rgba(255,255,255,0.45)", marginBottom: "1.25rem", position: "relative" }}>{t.tagline}</div>
        ...
        <div style={{ background: "rgba(255,255,255,0.07)", border: "0.5px solid rgba(255,255,255,0.13)", borderRadius: 16, padding: "0.875rem", position: "relative" }}>
              ...
              <div style={{ position: "absolute", left: 0, right: 0, top: "100%", zIndex: 100, background: "#0d1e35", border: "0.5px solid rgba(255,255,255,0.15)", borderRadius: 12, marginTop: 4, overflow: "hidden" }}>
                {fromResults.map((s) => (
                  <div key={s.id} onClick={() => { setFrom(s.name); setFromId(s.id); setFromResults([]); }}
                    style={{ padding: "10px 14px", cursor: "pointer", fontSize: 14, color: "rgba(255,255,255,0.85)", borderBottom: "0.5px solid rgba(255,255,255,0.06)" }}>
          ...
          <div style={{ display: "flex", gap: 6, marginBottom: 12 }}>
            {USER_TYPES.map((u) => (
              <button key={u.id} onClick={() => setUserType(u.id)}
                style={{ flex: 1, padding: "6px 4px", borderRadius: 8, border: "0.5px solid", borderColor: userType === u.id ? "#64b5f6" : "rgba(255,255,255,0.1)", background: userType === u.id ? "rgba(100,181,246,0.15)" : "rgba(255,255,255,0.05)", color: userType === u.id ? "#64b5f6" : "rgba(255,255,255,0.5)", fontSize: 11, fontWeight: 500 }}>
          ...
          <button onClick={handleSearch} disabled={!fromId || !toId}
            style={{ width: "100%", padding: "12px", borderRadius: 12, background: fromId && toId ? "#1976d2" : "rgba(255,255,255,0.08)", color: fromId && toId ? "#fff" : "rgba(255,255,255,0.3)", fontSize: 15, fontWeight: 500, display: "flex", alignItems: "center", justifyContent: "center", gap: 8 }}>
              ...
              return (
                <div key={route.id}
                  ...
                  <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 8 }}>
                    {i === 0 && <span style={{ background: "#1976d2", color: "#fff", fontSize: 10, fontWeight: 500, padding: "2px 8px", borderRadius: 20 }}>{t.best}</span>}
          ...
          <>
            ...
            {popular.map((p, i) => (
              <div key={i}
                ...
                style={{ background: "rgba(255,255,255,0.06)", border: "0.5px solid rgba(255,255,255,0.12)", borderRadius: 12, padding: "0.875rem 1rem", marginBottom: 8, display: "flex", alignItems: "center", gap: 10, cursor: "pointer" }}>
